...
module.exports = class {
	...
	async run (message) {
		...
		// Si el autor es un bot
		if(message.author.bot){
			return;
		}

		// Si el usuario está en el servidor, está invisble o no está en caché
		...
		// Revisar si el bot fué mencionado
		...
		if(!cmd && !customCommandAnswer && message.guild){
			return;
		...
		if(!data.userData.achievements.firstCommand.achieved){
			...
			await message.channel.send({ files: [
				{
					name: "unlocked.png",
					attachment: "./assets/img/logros/logro_unlocked2.png"
				}
			]});
		}
...
/**
 * xp
 * Está función añade xp al usuario
*/
...
async function updateXp(msg, data){
	...
	// Revisar si el usuario tiene tiempo de espera en la DB
	...
	// Recordar en la base de datos cuando dar xp al usuario nuevamente (3min)
	const toWait = Date.now()+60000;
	xpCooldown[msg.author.id] = toWait;
    
	// Obtener un número random entre 10 y 5
	const won = Math.floor(Math.random() * ( Math.floor(10) - Math.ceil(5))) + Math.ceil(5);
    
	const newXp = parseInt(points+won, 10);

	// Calcular cuanta XP falta para subir de nivel
	const neededXp = 5 * (level * level) + 80 * level + 100;

	// Revisar si el miembro sube de nivel
	if(newXp > neededXp){
